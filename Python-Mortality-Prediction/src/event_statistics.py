...
def encounter_count_metrics(events, mortality):
	...
	'''
	unique_counts = events[['patient_id','timestamp']].groupby(['patient_id']).timestamp.nunique()
	alive_counts = unique_counts.drop(mortality['patient_id'])
	dead_counts = unique_counts[mortality['patient_id']]
...
def record_length_metrics(events, mortality):
	...
	group = events[['patient_id','timestamp']].groupby('patient_id').agg([max,min])
	lengths = group['timestamp']['max'] - group['timestamp']['min']
	
	dead_lengths = lengths[mortality['patient_id']]
	dead_lengths = dead_lengths.dt.days
	alive_lengths = lengths.drop(mortality['patient_id'])
	alive_lengths = alive_lengths.dt.days
