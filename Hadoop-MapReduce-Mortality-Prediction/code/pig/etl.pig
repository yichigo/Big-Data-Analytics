...
deadevents = FOREACH deadevente GENERATE 
	events::patientid AS patientid,
...
alive_group = GROUP aliveevents BY events::patientid;
alive_group = FOREACH alive_group GENERATE
	group AS patientid,
	MAX(aliveevents.events::etimestamp) AS time_max;

aliveevents = JOIN aliveevents BY events::patientid, alive_group BY patientid;
aliveevents = FOREACH aliveevents GENERATE
	aliveevents::events::patientid AS patientid,
	aliveevents::events::eventid AS eventid,
	aliveevents::events::value AS value,
	0 AS label,
	DaysBetween(alive_group::time_max, aliveevents::events::etimestamp) AS time_difference;
...
filtered = FILTER filtered BY (time_difference >= 0) AND (time_difference <= 2000);
...
featureswithid = FOREACH featureswithid GENERATE
...
events_id = DISTINCT( FOREACH featureswithid GENERATE eventid );
events_id = ORDER events_id BY eventid;
sorted_id = RANK events_id;
all_features = FOREACH sorted_id GENERATE
	($0 - 1) AS idx,
	$1 AS eventid;
...
features = FOREACH features GENERATE
	featureswithid::patientid AS patientid,
...
maxvalues = FOREACH maxvalues GENERATE
	group AS idx,
...
features = FOREACH normalized GENERATE
	features::patientid AS patientid,
	features::idx AS idx,
	((float)features::featurevalue/maxvalues::maxvalue) AS normalizedfeaturevalue;
