...
object Main {
  ...
  def testClustering(phenotypeLabel: RDD[(String, Int)], rawFeatures: RDD[(String, Vector)]): (Double, Double, Double) = {
    ...
    val kmeans_model = new KMeans().setSeed(6250L).setK(N_Cluster).setMaxIterations(20)
    val kmeans = kmeans_model.run(featureVectors).predict(featureVectors)
    val kmeans_pred = features.map(_._1).zip(kmeans).join(phenotypeLabel).map(_._2)
    val kMeansPurity = Metrics.purity(kmeans_pred)

    // println("kmeans")
    // kmeans_pred.map(x => ((x._1, x._2), 1.0)).reduceByKey(_ + _).collect.foreach(println)
     ...
     * Remove the placeholder below after your implementation
     */
    val GM_model = new GaussianMixture().setSeed(6250L).setK(N_Cluster).setMaxIterations(20)
    val GM = GM_model.run(featureVectors).predict(featureVectors)
    val GM_pred = features.map(_._1).zip(GM).join(phenotypeLabel).map(_._2)
    val gaussianMixturePurity = Metrics.purity(GM_pred)

    // println("GMM")
    // GM_pred.map(x => ((x._1, x._2), 1.0)).reduceByKey(_ + _).collect.foreach(println)
     ...
     * Remove the placeholder below after your implementation
     */
    val SKM_model = new StreamingKMeans().setK(N_Cluster).setDecayFactor(1.0).setRandomCenters(10, 0.5, 6250L).latestModel()
    val SKM = SKM_model.update(featureVectors, 1.0, "points").predict(featureVectors)
    val SKM_pred = features.map(_._1).zip(SKM).join(phenotypeLabel).map(_._2)
    val streamKmeansPurity = Metrics.purity(SKM_pred)

    // println("SKM")
    // SKM_pred.map(x => ((x._1, x._2), 1.0)).reduceByKey(_ + _).collect.foreach(println)
  ...
  def loadRddRawData(spark: SparkSession): (RDD[Medication], RDD[LabResult], RDD[Diagnostic]) = {
    ...
    val medication_input = CSVHelper.loadCSVAsTable(spark, "data/medication_orders_INPUT.csv", "medication_orders")
    val labResult_input = CSVHelper.loadCSVAsTable(spark, "data/lab_results_INPUT.csv", "lab_results")
    val diagnostic_input = CSVHelper.loadCSVAsTable(spark, "data/encounter_dx_INPUT.csv", "encounter_dx")

    val medication_df = sqlContext.sql("SELECT Member_ID, Order_Date, Drug_Name FROM medication_orders")
    val labResult_df = sqlContext.sql("SELECT Member_ID, Date_Resulted, Result_Name, Numeric_Result FROM lab_results")
    ...
    val medication: RDD[Medication] = medication_df.rdd.map(x => Medication(x(0).toString, sqlDateParser(x(1).toString), x(2).toString.toLowerCase))
    val labResult: RDD[LabResult] = labResult_df.rdd.filter(x => (x(3) != null)).map(x => LabResult(x(0).toString, sqlDateParser(x(1).toString), x(2).toString.toLowerCase, (NumberFormat.getNumberInstance(java.util.Locale.US).parse(x(3).toString).doubleValue())))
