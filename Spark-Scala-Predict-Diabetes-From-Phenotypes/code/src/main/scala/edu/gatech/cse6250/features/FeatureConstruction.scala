...
object FeatureConstruction {
  ...
  def construct(sc: SparkContext, feature: RDD[FeatureTuple]): RDD[(String, Vector)] = {
    ...
    val feature_map = feature.map(x => x._1._2).distinct().collect.zipWithIndex.toMap
     ...
     * placeholder code
     */
    val n_features = feature_map.size
    val feature_group = feature.map(x => (x._1._1, (x._1._2, x._2))).groupByKey()

    val result = feature_group.map {
      case (patientID, features) =>
        val feature_value = features.toList.map { case (featureName, featureValue) => (feature_map(featureName), featureValue) }
        val feature_vector = Vectors.sparse(n_features, feature_value)
        (patientID, feature_vector)
    }
